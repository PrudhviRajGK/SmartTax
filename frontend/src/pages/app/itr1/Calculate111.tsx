...
const ITR1Calculate = () => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (grossSalary === 0) { setError('Gross salary cannot be zero. Please re-upload Form-16.'); setLoading(false); return; }
  ...
  useEffect(() => { if (canCalculate && !itr1State.calculated) handleCalculate(); }, []);
  ...
  if (!canCalculate) return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('c1.title')}</h1>
      <Card>
        <div className="text-center py-12">
          <p className="text-gray-600 dark:text-gray-400">{t('c1.complete_first')}</p>
          <Button onClick={() => navigate('/app/itr-1/review')} className="mt-4">{t('c1.go_review')}</Button>
        </div>
      </Card>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400">{t('c1.calculating')}</p>
        </div>
      </Card>
    </div>
  );
  ...
  if (error) return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <Button onClick={handleCalculate} className="mt-4">{t('common.retry')}</Button>
      </Card>
    </div>
  );
  ...
  if (!result) return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="text-center py-12">
          <p className="text-gray-600 dark:text-gray-400">{t('c1.no_result')}</p>
          <Button onClick={handleCalculate} className="mt-4">{t('c1.calc_now')}</Button>
        </div>
      </Card>
    </div>
  );
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {t('c1.last_calc')} {itr1State.lastCalculatedAt ? formatDateTime(itr1State.lastCalculatedAt) : '—'}
          </p>
        </div>
      ...
      <Card className="p-6">
        <div className="space-y-4">
          <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700">
            <span className="text-sm text-gray-600 dark:text-gray-400">{t('c1.salary_tax')}</span>
            <span className="text-xl font-semibold text-gray-900 dark:text-gray-100">{fmt(salaryTaxBeforeCess)}</span>
          </div>
          <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700">
            <span className="text-sm text-gray-600 dark:text-gray-400">{t('c1.cess')}</span>
            <span className="text-xl font-semibold text-gray-900 dark:text-gray-100">{fmt(cess)}</span>
          </div>
          <div className="flex justify-between items-center pt-2">
            <span className="text-base font-medium text-gray-900 dark:text-gray-100">{t('c1.total_liability')}</span>
            <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">{fmt(totalTaxLiability)}</span>
          </div>
        </div>
      </Card>
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button variant="secondary" onClick={() => navigate('/app/itr-1/salary')}>{t('c1.edit_salary')}</Button>
      </div>
    </div>
  );
};
