...
const PageHeader = ({ itrType, page }: { itrType: string; page: number }) => (
  <View style={S.header} fixed>
    <View style={S.headerLeft}>
      ...
      <Text style={S.brandName}>SmartTax</Text>
    </View>
  ...
  <Page size="A4" style={S.page}>
    <View style={S.coverPage}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        <View style={[S.logoBox, { width: 44, height: 44, borderRadius: 11 }]}>
          <Text style={[S.logoText, { fontSize: 20 }]}>S</Text>
        </View>
        <Text style={{ fontSize: 28, fontWeight: 700, color: GRAY1 }}>SmartTax</Text>
      </View>
      <Text style={{ fontSize: 10, color: GRAY4, marginBottom: 40 }}>Free · Accurate · Built for Every Indian</Text>
      ...
      <View style={S.coverBadge}>
        <Text style={S.coverBadgeText}>✓ Budget 2024 · Section 87A · Standard Deduction ₹75,000</Text>
      </View>

      <Text style={S.coverDisclaimer}>
        This document is a tax computation summary generated by SmartTax.
        It is not an official ITR filing or acknowledgement from the Income Tax Department.
        Verify all figures before submitting your ITR on incometax.gov.in.
      ...
      <Page size="A4" style={S.page}>
        ...
        <View style={S.cardGrid}>
          ...
          <View style={S.card}>
            ...
            <Text style={S.cardSub}>on taxable income {INR(taxableIncome, 0)}</Text>
          </View>
        ...
        <Text style={S.sectionNote}>
          Under the New Tax Regime you receive a flat ₹75,000 Standard Deduction (Sec 16).
        ...
        <View style={S.slabBox}>
          ...
          {taxableIncome <= 1_200_000 && (
            <View style={S.rebateBadge}>
              <Text style={S.rebateText}>✓ Section 87A Rebate applied — taxable income ≤ ₹12,00,000 · Full tax waived</Text>
            </View>
          )}
        </View>
        ...
        <View style={[S.tipBox, isRefund ? S.tipBoxGreen : S.tipBoxAmber]}>
          <Text style={[S.tipTitle, isRefund ? S.tipTitleGreen : S.tipTitleAmber]}>
            {isRefund ? '✓ Refund — What to do next' : '⚠ Balance Payable — What to do next'}
          </Text>
          <Text style={[S.tipText, isRefund ? S.tipTextGreen : S.tipTextAmber]}>
            {isRefund
              ? 'Your TDS exceeds your tax liability. The refund will be credited to your pre-validated bank account after ITR processing — typically within 30–45 days of filing. Ensure your bank account is linked and validated on incometax.gov.in.'
              : `You owe ₹${INR(Math.abs(netPayable))} in additional tax. Pay via Challan 280 on the Income Tax e-filing portal BEFORE submitting your ITR to avoid interest under Sec 234B / 234C. Select "Self Assessment Tax" as the payment type.`
            }
          </Text>
        </View>
    ...
    <>
      ...
      <Page size="A4" style={S.page}>
        ...
        <View style={S.cardGrid}>
          ...
          <View style={S.card}>
            <Text style={S.cardLabel}>Effective Tax Rate</Text>
            <Text style={S.cardValue}>{PCT(effRate)}</Text>
            <Text style={S.cardSub}>on total income {INR(totalGross, 0)}</Text>
          </View>
          <View style={[S.card, isRefund ? S.cardGreen : S.cardRed]}>
            <Text style={S.cardLabel}>{isRefund ? 'Refund Due' : 'Balance Payable'}</Text>
            <Text style={[S.cardValue, isRefund ? S.cardGreenValue : S.cardRedValue]}>
        ...
        <Text style={S.sectionNote}>
          New Tax Regime: Flat ₹75,000 Standard Deduction. No 80C / HRA / NPS deductions permitted.
        ...
        <View style={S.slabBox}>
          ...
          {taxableIncome <= 1_200_000 && (
            <View style={S.rebateBadge}>
              <Text style={S.rebateText}>✓ Section 87A Rebate — taxable income ≤ ₹12,00,000 · Full tax waived</Text>
            </View>
          )}
        </View>
        ...
        <Page size="A4" style={S.page}>
          ...
          {hasEquity && (
            <>
              ...
              <Text style={S.sectionNote}>
                ...
                First ₹1,25,000 of total LTCG (equity + equity MF combined) is exempt under Sec 112A.
              </Text>
              <View style={S.table}>
                <View style={S.tableHead}>
          ...
          {hasMF && (
            <>
              ...
              <Text style={S.sectionNote}>
                Equity MFs: STCG taxed at 20%, LTCG at 12.5% after ₹1,25,000 exemption (Sec 112A).
              ...
              <View style={S.table}>
                ...
                {eqLtcg > 0 && (
                  <>
                    ...
                    <View style={S.tableRow}>
                      <Text style={[S.tableCell, { flex: 1 }]}>  Less: LTCG Exemption Sec 112A (₹1,25,000)</Text>
      ...
      {hasHP && (
        <Page size="A4" style={S.page}>
          ...
          <Text style={S.sectionNote}>
            Under the New Tax Regime: SOP has NAV = ₹0 (no income, no Sec 24b deduction).
                ...
                {inp.property_type === 'SOP' ? (
                  <View style={{ padding: 12 }}>
                    <Text style={{ fontSize: 8.5, color: GRAY3 }}>
                      Self-Occupied Property — Gross Annual Value = ₹0 under New Tax Regime.
                      No deductions applicable.  HP Income = ₹0.
                    </Text>
                  </View>
                ) : (
                  <View>
        ...
        {(stcgAfter > 0 || eqStcg > 0) && (
          <View style={[S.tipBox, { backgroundColor: '#fffbeb', borderColor: '#fcd34d', marginTop: 10 }]}>
            ...
            <Text style={[S.tipText, { color: '#78350f' }]}>
              The Income Tax Department has clarified that Sec 87A rebate does NOT apply to STCG taxed at 20% (post-July 2024) and LTCG under Sec 112A. This computation applies 87A only to slab income (salary head). If your taxable salary income alone is ≤ ₹12,00,000, the rebate is applied there. Verify on the portal — this has been subject to legal challenge.
            </Text>
          </View>
        )}
